...
class TestUpdateCutDescriptionAPIView:
    """
    This class encapsulates the tests for the view responsible for updating
    descriptions of a cutting technique.
    """

    path_name = "update_cut_descriptions"
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            (
                "router_cnc",
                {
                    "about_text": "Otra descripción del servicio.",
                    "card_text": "Otra descripción de la tarjeta servicio.",
                    "common_uses_text": "Otros usos comunes del servicio.",
                    "main_text": "Otro texto principal del servicio.",
                },
            )
        ],
        ids=["valid_data"],
    ...
    def test_if_update_cut(
        ...
        assert response.data["about_text"] == request_data["about_text"]
        assert response.data["card_text"] == request_data["card_text"]
        assert response.data["common_uses_text"] == request_data["common_uses_text"]
        assert response.data["main_text"] == request_data["main_text"]
        # fmt: on
